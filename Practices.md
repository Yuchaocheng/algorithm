 ...
 * @LastEditTime: 2022-05-10 10:34:35
-->
...
- 分发饼干：小饼干优先喂小胃口的小朋友，这样就能保证尽可能满足更多小朋友的胃口，并且生下的饼干尽可能多

  - 解题：先将饼干和小朋友排序，然后就可以按顺序匹配了。局部最优指的就是小饼干喂小胃口

- 摆动序列：需要计算最长摇摆序列，其实就是让序列有尽可能多的局部峰值

  - 局部最优：删除单调坡度上的节点（不包括单调坡度两端的节点），那么这个坡度就可以有两个局部峰值
  - 整体最优：整个序列有最多的局部峰值，从而达到最长摆动序列
  - 代码核心：通过比较前一份差值和当前份差值，前一份差值可以等于 0，后一份差值必须和前一份正负相反

- 最大子序列：连续子序列和最大
  - 解题思路：当前“连续和”为负数的时候立刻放弃，从下一个元素重新计算“连续和”，因为负数加上下一个元素 “连续和”只会越来越小。从而推出全局最优：选取最大“连续和”
  - 代码思路：遍历以此数组，然后叠加，一旦叠加值小于零就重新开始计算
