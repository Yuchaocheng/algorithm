...
- 一般来说在线性结构中认为前驱即上一个节点、后继即下一个节点。但是比如在二叉树种，前驱为该比该节点小的最大节点
...
5. 二维数组实际是一个线性数组存放着其他数组的首地址，所以二维数组内存地址不是连续的
...
时间复杂度指的是执行时间与输入数值之间的关系

一般程序中有 for 循环，比如循环 n 次，那么时间复杂度就是 O(N)，一般的语句时间复杂度就是 O(1)，因为一般的语句不会随着数据量的增大而多执行
...
空间复杂度指的是存储空间和输入值之间的关系

一般考虑两点：

- 1 变量：
  如果是简单类型，空间复杂度为 O(1),因为简单类型始终存储的就那么一块内存
  如果是复杂类型，比方说是数组，那么就要看具体的代码，如果在循环中不断往数组中 push，可能就是 O(N)

- 2 递归函数
  递归函数本身内存就需要给它分配一定空间，放到执行栈中，所以递归的次数如果随着输入大小改变，那么它的空间复杂度也可能是 O(N)
...
- 冒泡排序相对其他排序运行效率较低，但在理解上市最简单的，所以学习时优先学习

- 冒泡排序的思路（升序）：

  1. 对未排序的各元素从头到尾依次比较相邻两个元素大小
  2. 如果左边的值大，则交换位置，如果右边的值大，则已符合顺序，比较下两个大小
  3. 按照这个规律，一直到最后比较 length-2 和 length-1 项
  4. 走完一次后，末尾的项则为最大值
  5. 继续根据前 4 步的步骤，进行第二轮循环。已经比较过的不用再比较。

- 图解：
  ![冒泡排序过程](./images/冒泡排序过程.gif)

- 总结：
  需要进行双重循环，每一次内部循环较少一次次数减少。所以效率比纯双重循环好一点。
  纯双重循环即从数组中找到最小项，放入一个数组中，然后找出第二个。

- 冒泡排序的效率：
  假设有 7 个数据使用冒泡排序进行排序
  1. 比较次数为：6 + 5 + 4 + 3 + 2 + 1
  2. 对于 N 个数据项，比较次数为 N \* (N - 1) / 2，大 O 表示法为 O(n^2)
  3. 交换次数是根据原数组或者队里的顺序决定的，比如极端一点，刚好按顺序排列，则一次都不需要交换
     这里如果取平均值，比较两次交换一次顺序
     则交换次数为：N \* (N - 1) / 4，大 O 表示法还是 O(n^2)

#### 选择排序

- 选择排序改进了冒泡排序：

  - 将交换次数由 O（N^2）减小到 O（N）
  - 但是比较次数依然是 O（N^2）

- 思路：

  1. 记录每一轮的最小位置索引，比如假设第一轮位置 0 最小（第二轮应该假设 1 位置最小）
  2. 然后遍历数组（未排列过的），依次将数组元素和最小位置元素比较
  3. 如果当前元素比最小位置小，则该最小位置改变为该位置
  4. 这样一轮下来，就能找出该轮中的最小位置，将该位置放到未排列数组的开头
  5. 一次进行 length-1 次这样的排列，即可将数组排列完毕

- 效率：
  - 选择排序的比较次数和冒泡排序一样，为：N \* (N - 1) / 2，用大 O 表示法表示为：O（N^2）
  - 选择排序的交换次数为：(N - 1) / 2，用大 O 表示法表示为：O（N）
  - 所以选择排序的效率高于冒泡排序

#### 插入排序

插入排序是简单排序中效率最高的一种排序。并且插入排序是学习其他高级排序的基础。

- 思想：

  - 插入排序思想的核心是局部有序，假设部分左侧的人是有序的
  - 首先指定一数据 X（从第一个数据开始），并将数据 X 的左边变成局部有序状态
  - 随后将 X 右移一位，再次达到局部有序之后，继续右移一位，重复前面的操作直至 X 移至最后一个元素

- 图解：
  ![插入排序过程](./images/插入排序过程.gif)

- 效率

  - 比较次数
    第一轮时，需要的最大次数为 1；第二轮最大为 2，，最后一趟最大为 N-1
    所以插入排序的最大总比较次数为 N \* (N - 1) / 2，而冒泡和选择排序的比较次数一定是 N \* (N - 1) / 2
    插入排序的平均比较次数为：N \* (N - 1) / 4

  - 交换次数
    第一轮时，最多交换 1 次，第二轮最多 2 次，最后一轮为 N-1 次
    所以最多需要交换 N \* (N - 1) / 2 次，平局交换次数为 N \* (N - 1) / 2

  - 虽然用大 O 表示法表示插入排序的效率也是 O（N^2），但是插入排序整体操作次数更少，因此，在简单排序中，插入排序效率最高

- 总结：
  - 对于三种简单排序来说，交换次数肯定是和数组本来的顺序是有关系的。比如本来就是按顺序的，则不需要交换。
  - 并且对于插入排序来说，其实是移动和插入的过程，严格意义上不属于交换，效率略高于交换。
  - 但是对于比较次数来说，插入排序最少只需要比较 N-1 次，而冒泡排序和选择排序则是固定 N \* (N - 1) / 2，该次数为插入排序的最大排序次数
...
- 历史背景
  ...
  - 在希尔算法出现之前，各种各样的排序算法时间复杂度都是 O（N^2），计算机界充斥着排序算法不可能突破 O（N^2）的声音
  - 希尔算法首次突破了 O（N^2）的大关，为了纪念该算法里程碑式的意义，用 Shell 来命名该算法
  - 当希尔算法出现后，紧接着出现了好几种超越 O（N^2）的排序算法，比如后面要学的快速排序
...
- 希尔算法的实现思路
  ...
  - 根据设定的增量（gap）将数据分为 gap 个组（组数等于 gap），再在每个分组中进行局部排序
    这里的增量是指，第一组的组员为 0,0+gap，0+2gap......直到数组末尾
    为什么 gap 是多少就有多少组呢，因为会被分成 0+1+2+...gap-1 个组，0~gap-1 肯定不是同一个组的组员
  - 排序之后，减小增量，继续分组，再次进行局部排序，直到增量 gap=1 为止。随后只需进行微调就可完成数组的排序

- 思路理解：为什么这样有助于提高效率

  - 一步一步优化数组中的顺序，有利于插入排序的运行
  - 假设最小的一个数在末尾，那么插入排序执行到它时，便要执行 length-1 次的比较和移动
  - 而如果使用希尔算法，在第一次分组中，只比较了一次就排列到了数组的中部位置
  - 这样下一次排列时的效率就会提高，通过不断提高效率完成了本次排序

- 图解：
  ![希尔排序过程](./images/希尔排序过程.gif)

- 增量的选择：

  - 原稿中希尔建议的初始间距为 N / 2，比如对于 N = 100 的数组，增量序列为：50，25，12，6，3，1，不能整除时向下取整
  - Hibbard 增量序列：增量序列算法为：2^k - 1，即 1，3，5，7... ...等；这种情况的最坏复杂度为 O（N^3/2）,平均复杂度为 O（N^5/4）但平均复杂度未被证明

  - Sedgewcik 增量序列
    是给定的一种序列，仅做了解

  - 希尔排序根据增量的不同，时间复杂度会有所不同

- 希尔排序的效率：
  - 希尔排序的效率和增量有直接关系，即使使用原稿中的增量效率都高于简单排序
  - 但希尔排序的效率证明很困难，可使用实际运行程序比较。大量的统计也证明了这一点
  - 甚至在合适的增量和某些 N 的情况下，还好与快速排序，但是大多数情况下，还是快速排序效率高
...
- 介绍
  ...
  - 快速排序被列为 20 世纪十大算法之一，非常重要

- 思想

  - 快速排序的核心思想是分而治之
  - 先选出一个数据（比如 65），将比其小的数据都放在它的左边，将比它大的数据都放在它的右边。这个数据称为枢纽

- 和冒泡排序的不同：

  - 我们选择的 65 可以一次性将它放在最正确的位置，之后就不需要做任何移动
  - 而冒泡排序即使已经找到最大值，也需要继续移动最大值，直到将它移动到最右边

- 快速排序的枢纽

  - 第一种方案：直接选择第一个元素作为枢纽。但是，当第一个元素就是最小值或最大值的情况下，效率不高
  - 第二种方案：使用随机数。随机数的生成本身就比较耗费性能，不推荐
  - 优秀的解决方法：取 index 为头、中、尾的三个数据排序后的中位数

- 快速排序的效率

  - 快速排序最坏情况下的效率：每次选择的枢纽都是最左边或最右边的数据，此时效率等同于冒泡排序，时间复杂度为 O（n2）。可根据不同的枢纽选择避免这一情况

  - 快速排序的平均效率：为 O（N*logN），虽然其他算法效率也可达到 O（N*logN），但是其中快速排序是最好的快速排序的平均效率：为 O（N*logN），虽然其他算法效率也可达到 O（N*logN），但是其中快速排序是最好的
...
- 数组
  ...
  - 优点：可以通过下标值访问，效率非常高
  - 缺点：查找、插入和删除效率都很低；查找数据时需要先对数据进行排序，生成有序数组，才能提高查找效率；并且在插入和删除元素时，需要大量的位移操作
  - 应用场景：
    适合读、不适合频繁增删操作。读多写少
...
- 链表：
  ...
  - 优点：插入和删除操作效率都很高。注意这里的插入和删除指的是行为本身
    举例如果在链表中间插入一个元素，查到需要 N/2 次，但是插入只需要断开指针+建立指针即可完成
    对比数组中间插入元素，查找根据下标一次完成，但是插入后，后面所有的元素都需要调整一次，执行了 N/2 次
  - 缺点：查找效率低，需要从头开始依次查找，直到找到目标数据为止；当需要在链表中间位置插入或删除数据时，插入或删除的效率都不高，因为先查找后插入或者删除
...
- 哈希表：
  ...
  - 空间利用率不高，底层使用的数组中很多单元没有被利用；并且哈希表中的元素是无序的，不能按照固定顺序遍历哈希表中的元素。所以哈希表中想要查找最大值或最小值比较困难
...
- 概念：回溯法也可以叫做回溯搜索法，它是一种搜索的方式

  - 回溯是递归的副产品，只要有递归就会有回溯
  - 回溯函数：也就是递归函数，指的都是一个函数
  - 个人理解：栈也存在回溯，入栈的时候是第一次相遇，出栈的时候就是回溯了。肯定存在一定场景下，栈可以模拟回溯

- 回溯法的效率：

  - 回溯法并不是什么高效的算法，因为回溯的本质是穷举，穷举所有可能，然后选出我们想要的答案
  - 如果想让回溯法高效一些，可以加一些剪枝的操作，但本质还是穷举
  - 效率不高为什么还要用，因为没得选，有些问题只能穷举
...
- 排列和组合的差异：一句话组合无序，排列有序：{1, 2} 和 {2, 1} 是一个组合但是是两种排列

- 如何理解回溯法： 回溯法解决的问题（所有问题）都可以抽象为树形结构

  - 回溯法解决的都是在集合中递归查找子集：集合构成树的宽度，递归构成树的深度
...
- 概念： 动规是由前一个状态推导出来的，而贪心是局部直接选最优的
